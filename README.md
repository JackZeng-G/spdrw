...
用 Go 写的 SPD 读写工具: Windows 桌面程序, 通过 **PawnIO** 内核驱动直连芯片组 SMBus,
读取/解析/编辑/写入内存条 SPD。支持 DDR3 / DDR4 / DDR5,**不支持 DDR2**。
...
- **原始数据区行尾参数标注 + 全字节悬停说明**: 每行 hex 行尾直接标出该行起始的参数名
  (身份/时序/XMP/布局说明), 悬停标注高亮它覆盖的字节; 悬停任意字节弹出说明 ——
  DDR3/DDR4/DDR5 每个字节都有名字, 没有"未映射"死角(回归测试钉死)
- **模块/DRAM 厂商改成常用厂商下拉**: 预设 29 家(Samsung/SK Hynix/Micron/Kingston/
  金百达/光威/朗科/长鑫…), 选中自动写入对应 JEP106 ID(含奇校验位, 写入口径与
  58 份真实语料逐条一致); ID 不硬编码, 运行时按名字回查 JEP106 表, 不会给错 ID
- **聚焦字段即在 hex 里闪高亮定位**: JEDEC 时序字段的注记偏移(`[3:0]` nibble、
  `(ps)/(12b)` 单位)也支持 —— 修复了时序字段此前定位不了、悬停不出名字的问题
- 修复: 重扫误报 `ReferenceError`(扫描实际已完成); 探测自愈"页残留"(上个软件把
  EE1004 留在页 1 导致 DDR4 被误判成 256B/未知世代)

更早的 v1.0.0 → v1.0.1 变化: 时序按周期(clk)查看与输入(`16clk` 自动 ×tCK 换算)、
编辑提交后各视图即时同步、备份目录改到程序同级 `backup\`、日志去重、页寄存器写后归零等 ——
完整清单见 [docs/实现文档.md](docs/实现文档.md)。

## 功能

### 读得快

- **忙等等待模式**: PawnIO 模块把每次事务的等待交给 Windows 线程休眠时, 一次等待就是一个
  时钟中断(约 15.6ms)→ 每次事务固定 ~31ms、整片 1024B 要 16 秒。本工具默认用**忙等**
  (µs 精确), 并把模块的 SMBus 时钟读出来显示(真机实测 392.9 kHz); 1024B 整片读取因此从
  16.5 秒降到**约 0.3 秒**。无需手动开关: 自动选忙等, 出错时自动降级为"轮询忙等+长等待休眠"。
- **三级自适应读取**: **SMBus Block Read**(协议 5, 32 字节/事务) → **Word Read**(2 字节/事务)
  → 逐字节。每一级都先只读探测、失败即降级(AMD FCH 实测会被 HUB 拒绝块读, 于是自动落到字读:
  真机 1024B = 512 次事务 ≈ 0.32 秒)。日志与"SPD 信息"会写明实际档位、事务数、耗时与回退原因。

### 看得懂

- **控制器发现**: Intel PCH (I801) / AMD FCH (PIIX4, 双端口) / Intel Skylake-SP IMC (SKX, 双总线);
  控制器列表只列出**探测到 SPD 的**端口(AMD FCH 的 5 个端口里只有实际接条的那个有意义)。
- **SPD 扫描** 0x50-0x57, DDR4(EE1004 SPA0/SPA1 快速命令分页)与 DDR5(MR11 寄存器分页)自动识别
- **解析**:
  - DDR4/LPDDR3/4/4X: 模块类型、密度/Bank/行列、组织、位宽、容量、全部主时序(中+细粒度)、
    CAS 掩码、双段 CRC(含修复)、厂商/日期/序列号/部件号、**XMP 2.0 双 Profile**(时序换算+电压)
  - DDR5/LPDDR5(X): 密度/组织/通道/位宽、身份区、**完整 JEDEC 时序**(byte 20-102, ps/ns +
    lower limit)、CRC(**基础段 + XMP 3.0 header/各槽 + EXPO**)、按规范修正的 XMP 3.0 槽位
    (0x2C0..0x3C0)
  - DDR3: 容量/组织/部件号/厂商/日期/序列号/校验
  - **不支持 DDR2**(太老, 找不到真实 dump 用于验证; byte2=0x08-0x0A 按"未知类型"拒绝解析与写入)
- **hex 视图三套正交的视觉信号**(说明都收在 hex 标题的"?"里):
  **区段底色** = 参与校验(蓝) / 不参与校验(绿) / 身份信息(紫) / JEDEC 时序(青) / XMP-EXPO(黄);
  **改动标记** = 红底(影响校验, 须重算 CRC) / 蓝底(序列号、日期等不影响校验) / 黄框(CRC 值本身);
  **行尾参数标注** = 每行行尾列出该行起始的参数名, 悬停标注高亮它覆盖的字节(跨行)

### 改得准

- **SPD 编辑器**(内存中编辑, 不落盘不写设备):
  - 常用信息: 厂商(**常用厂商下拉**, 选中自动写对应 JEP106 ID)、厂商码、生产地点、日期、
    序列号、部件号、修订码、DRAM 厂商(同款下拉)/stepping
  - JEDEC 时序: DDR4 medium+fine、DDR5(ps/ns, 含 lower limit)、DDR3 MTB/FTB;
    **按时间或周期两种写法**: 输入 `10`(ns)或 `16clk`(自动 ×tCK 换算), 字段旁实时显示折算
    周期与 `1clk = X ns · Y MT/s`; XMP/EXPO 的 profile 时序按该 profile 自己的 tCK 换算
  - 扩展信息: **XMP 2.0**(DDR4, 2×63B)、**XMP 3.0**(DDR5, header + 5 槽)、**EXPO**(DDR5,
    与 XMP 槽 3 互斥) —— 可创建/修改/清除, CRC 自动重算
  - hex 视图内点格子就地编辑任意字节; **实时校验状态**与**重算 CRC**都在标题行
  - 字段两列网格, 默认只显示常用字段, 可"显示全部字段"; **聚焦/点击任意字段输入框, 左侧 hex
    里对应字节闪高亮定位**
  - **读一次即自动载入编辑器**; 写保护整块放在编辑器页; 文件操作(打开 dump/另存为/校验)在顶栏
...
- **自动备份**当前整片到程序同级 `backup\` 目录; 确认串 `WRITE` 才真正执行
- 写入计划按字节 diff(**增量模式**, 只写与设备不同的字节并回读校验, 可强制全量),
  **CRC 字节最后写** —— 中断只会留下 CRC 不符的 SPD, 而不是"看着有效但内容错"
- 中途失败给出"已写/未写字节数 + 恢复建议"; 逐字节回读校验; **写完自动三层校验 + 独立复核**
- **干跑**: 完整跑预检/计划/校验, 一个字节都不上总线(日志里"对 SPD NVM 的数据写 0 次"可验证)
- **写入能力探测**: 先在保留字节上做一次"写 → 读回 → 还原", 证明平台真能写, 再动真正的字段

### 写保护

- RSWP 状态检测: DDR5 读 MR12/MR13 位图(16×64B)、DDR4 及更早块首写测试(4×128B,
  **还原后回读确认**, 失败重试并报"状态未知", 绝不谎报); RSWP 按块设置/清除
- PSWP: 仅 DDR3 可经 PWPB(0110b)探测; DDR4/DDR5 无该设备类型, 明确显示"不适用"
- 显示 BIOS "SPD write disable" 状态(I801); 锁定时常驻**只读模式**横幅并提示去 BIOS 找开关;
  平台锁定时自动跳过写测试/写入能力探测, 一个字节都不写
...
go test ./internal/...                          # 173 个顶层用例
CGO_ENABLED=1 go test -race ./internal/...      # 并发/死锁问题只有 race 抓得到
node --test "frontend/test/*.test.mjs"          # 前端契约与漂移守卫(51 条)
...
- 真实 dump 语料(58 份: 29×DDR3 + 18×DDR4 + 11×DDR5)与其来源清单见 `testdata/spd/MANIFEST.md`。
