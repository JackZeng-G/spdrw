...
2026-09-22 增补: ⑥ 原始数据区行尾参数标注 + 全字节悬停说明(DDR3/4/5 无死角);
⑦ 模块/DRAM 厂商改**常用厂商下拉**, 选中自动写 JEP106 ID(含奇校验位, 与真机语料一致);
⑧ 聚焦字段在 hex 里闪高亮定位(时序字段的注记偏移已支持); ⑨ 重扫误报 ReferenceError 修复;
⑩ 探测自愈"页残留"(上个软件把 EE1004 留在页 1 导致 DDR4 被误判 256B)。
...
- **SPD 编辑器**(内存中编辑, 不落盘不写设备):
  - 常用信息: 厂商(**常用厂商下拉**, 选中自动写对应 JEP106 ID)、厂商码、生产地点、日期、序列号、
    部件号、修订码、DRAM 厂商(同款下拉)/stepping
  ...
  - 字段两列网格, 默认只显示常用字段(JEDEC 时序的关键项/第 1 份 profile), 可"显示全部字段";
    **聚焦/点击任意字段输入框, 左侧 hex 里对应字节闪高亮定位**(含带注记偏移的 JEDEC 时序字段:
    `[3:0]` nibble 按整字节定位, `(ps)/(12b)` 单位注记自动剥除)
...
- hex 视图有**三套正交的视觉信号**(说明都收在 hex 标题的"?"里):
  **区段底色** = 参与校验(蓝) / 不参与校验(绿) / 身份信息(紫) / JEDEC 时序(青) / XMP-EXPO(黄);
  **改动标记** = 红底(影响校验, 须重算 CRC) / 蓝底(序列号、日期等不影响校验) / 黄框(CRC 值本身);
  **行尾参数标注** = 每行行尾列出该行起始的参数名(身份/时序/XMP/布局说明), 悬停标注会高亮它覆盖
  的字节(跨行); 悬停任意字节弹出参数说明 —— 三个世代的每个字节都有名字, 没有"未映射"死角
  (回归测试 `TestInfoFieldsCoverEveryByte` 把这句话钉死)
- **写入护栏**(写入是唯一可能变砖的操作):
