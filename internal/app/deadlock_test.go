...
// TestNoSelfDeadlockInBoundMethods 机械守卫: 每个"持操作锁"的入口(含已不对外暴露的内部入口)
// 都必须在
...
func TestNoSelfDeadlockInBoundMethods(t *testing.T) {
	cases := []struct {
		...
		{"busStats(内部)", func(a *App) error { _, err := a.busStats(); return err }},
		{"preflightWrite(内部)", func(a *App) error {
			_, err := a.preflightWrite("/nonexistent-file", false)
			return err
		}},
		{"writeConfirmed(内部)", func(a *App) error {
		...
		{"verifyFile(内部)", func(a *App) error { return a.verifyFile(filepath.Join(t.TempDir(), "x.bin")) }},
		{"editLoadPath(内部)", func(a *App) error { _, err := a.editLoadPath(filepath.Join(t.TempDir(), "x.bin")); return err }},
